// Tarefas periódicas dos Fluxos de automação — chamadas diretamente pelo
// agendador em processo (server/_core/scheduler.ts), sem round-trip HTTP.
...
export async function retomarFluxosPendentes() {
  try {
    ...
    return { ok: false as const, error: error instanceof Error ? error.message : "erro desconhecido" };
  }
}

export async function dispararFluxosAgendados() {
  try {
    ...
    return { ok: true as const, disparados };
  } catch (error) {
...
export async function alertarBuddhaMktSemRetorno() {
  try {
    ...
    return { ok: true as const, alertados };
  } catch (error) {
