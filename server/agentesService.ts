...
export function instrucaoContextoRelacionamento(chaveAgente: string) {
  ...
  // "Por favor, aguarde um momento ✨" era a saída ao terminar a coleta, mas
  // não existe hoje nenhuma automação que resolva algo nesse "aguarde" — a
  // mensagem chegava vazia de conteúdo pro cliente e, pior, se repetia a
  // cada nova mensagem dele enquanto a recepção não respondia (2026-08-30:
  // 9 sugestões idênticas em 32 minutos numa mesma conversa). Terminar a
  // coleta agora cai na mesma saída silenciosa da REGRA DE NÃO INTERVENÇÃO
  // (instrucaoNaoIntervencao) — sem card novo no Inbox a cada mensagem. Se
  // um dia existir automação real que aja sobre os dados coletados,
  // "aguarde um momento" pode voltar a fazer sentido como status genuíno.
  return "\n\nCONTEXTO E FLUXO OPERACIONAL DA CAROL: você prepara o agendamento para que a recepção consulte a agenda somente uma vez; não consulta, não confirma e não promete vaga. Nunca afirme que existe horário. O contexto informa exclusivamente a mesma unidade da conversa. Se houver plano ativo, presuma o uso do plano e não pergunte se o cliente quer utilizá-lo; use saldo de sessões apenas como referência factual para o resumo. Se houver última terapia e o cliente informou data/horário ou profissional, mas não informou a terapia, pergunte: \"A terapia será [última terapia]?\"; nunca presuma sem a confirmação. Se não houver última terapia ou se a terapia não estiver clara, pergunte de forma aberta: \"Qual será a terapia?\". Se o cliente pedir explicação, comparação ou indicação entre terapias, responda com status \"bianca\" e preserve as variáveis coletadas. A quantidade de atendimentos concluídos serve apenas para distinguir histórico conhecido na unidade: só depois de completar a coleta de agenda, se ela for zero, selecione o Script \"Solicitação de dados para agendamento\" (ID 30020). A falta de cadastro nunca interrompe a coleta. BLOQUEIO DE ETAPA: antes de formular uma pergunta, confira a última mensagem e as variáveis já coletadas. Pergunte somente o próximo dado realmente ausente. Nunca repita pergunta já respondida, não retome a coleta quando o cliente estiver agradecendo ou encerrando e não crie nova pergunta depois de a solicitação estar completa. Quando a recepção precisar apenas consultar ou confirmar a agenda, use a saída silenciosa de não intervenção; não envie despedida nem promessa ao cliente. Pergunte sobre preferência de terapeuta somente se ela ainda estiver ausente e for relevante; se o cliente já informou profissional, gênero ou indiferença, registre e siga sem repetir. Ao terminar a coleta, registre no summary dia, horário/período, terapia com duração, profissional de preferência se houver, pessoa atendida, plano/voucher e cadastro pendente se aplicável, e use exatamente a saída silenciosa de não intervenção (status \"failure\", message vazio) — não crie mensagem visível, não diga \"aguarde um momento\" nem repita esse encerramento em mensagens seguintes do cliente enquanto a recepção não responder. Não faça pergunta adicional nesse ponto.";
}
...
function textoContexto(contexto: ContextoConversa) {
  ...
  return `${cabecalho}\n\nHistórico recente:\n${historico}${resumoBelle ? `\n\nDados espelhados do cliente (referência operacional):\n${resumoBelle}` : ""}`;
}
