...
function registerZapiWebhook(app: Express) {
  app.post("/api/webhooks/zapi/:unidadeId", async (req: Request, res: Response) => {
    try {
      ...
      // Copilot de agentes: no estágio inicial, a classificação e a
      // sugestão são processadas na própria chegada da mensagem, mas a
      // resposta continua aguardando a validação humana na fila. Áudios
      // só entram após a transcrição, para que o especialista receba texto.
      if (!payload.isGroup && !payload.fromMe && mensagemId && tipo !== "audio") {
        try {
          ...
          console.error("[Webhook Z-API] Falha ao processar agentes:", error);
        }
      }
        ...
        transcribeAudio({ audioUrl: payload.audio.audioUrl, language: "pt" })
          .then(async (result) => {
            if ("text" in result) {
              ...
              const transcricao = result.text.trim() || "(sem fala identificada)";
              await db.updateInboxMensagemTranscricao(mensagemId, transcricao);
              if (!payload.isGroup && !payload.fromMe) {
                const { processarMensagemRecebida } = await import("./agentesService");
                await processarMensagemRecebida({ conversaId, mensagemEntradaId: mensagemId });
              }
      ...
      // no roteador de unidade). Nunca bloqueia a resposta do webhook.
        ...
        if (mensagemId) {
          try {
            ...
            console.error("[Webhook Buddha Mkt] Falha ao processar agentes:", error);
          }
        }
      }
