...
export const fluxoExecucoes = mysqlTable("fluxo_execucoes", {
  ...
  // TiDB não aceita DEFAULT em colunas JSON; as inserções de fluxo já
  // fornecem um objeto vazio quando não há variáveis definidas.
  variaveis: json("variaveis").$type<Record<string, string>>().notNull(),
...
/**
 * Catálogo configurável dos agentes de atendimento. O receptor escolhe
 * apenas entre agentes especialistas ativos; cada agente pode operar em
 * modo assistido ou automático de forma independente.
 */
...
/**
 * Configuração operacional de cada agente por unidade. O catálogo de agentes
 * permanece global, mas modo, modelo e ativação nunca vazam entre unidades.
 */
...
/**
 * Histórico imutável dos prompts. A aplicação garante apenas uma versão
 * ativa por agente e preserva autor, data e status de cada alteração.
 */
...
/** Estado persistido de uma conversa enquanto estiver sob atendimento IA. */
...
/** Conteúdo oficial, versionável e restrito a uma unidade, fora dos prompts. */
...
/** Tabela comercial pesquisável; substitui preços soltos dentro dos prompts. */
...
/** Impede o reenvio de um mesmo ativo ou ação operacional na conversa. */
...
/**
 * Rastreia o encaminhamento de cada mensagem recebida: receptor, destino,
 * classificação e eventual erro. Não armazena conteúdo bruto fora do
 * histórico já existente do Inbox.
 */
...
/**
 * Sugestão persistida para a recepção. A decisão humana é separada do
 * envio para que as métricas continuem corretas após uma resposta aprovada
 * ser efetivamente enviada ao WhatsApp.
 */
