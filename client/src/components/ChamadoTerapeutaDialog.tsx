...
import { BellRing, CheckSquare, Clock3, Loader2, Send, Shuffle, X } from "lucide-react";
    ...
    <DialogContent className="max-h-[92dvh] max-w-2xl overflow-y-auto">
      ...
      {!destinoTesteDisponivel ? <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800">O envio de teste está configurado somente para o grupo da recepção do Ribeirão Shopping.</div> : opcoesQuery.isLoading ? <div className="flex items-center gap-2 rounded-lg border p-4 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Preparando opções do chamado...</div> : <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <CampoBuscaLista label={form.modalidade === "pre_chamado" ? "Como preparar" : "Aguardando em"} value={form.aguardandoEm} onChange={(valor) => mudar("aguardandoEm", valor)} valores={aguardando.map((item) => item.nome)} placeholder="Informe onde o cliente está" id="chamado-aguardando" />
          <div className="flex items-end gap-2"><div className="min-w-0 flex-1"><CampoBuscaLista
            label="Terapeuta"
            labelExtra={
              <Tooltip>
          ...
          <CampoBuscaLista label="Terapia Bem-Estar" value={form.terapiaBemEstar} onChange={(valor) => mudar("terapiaBemEstar", valor)} valores={nomesBemEstar} placeholder="Selecione ou digite" id="chamado-bem-estar" />
          <CampoBuscaLista label="Terapia Estética" value={form.terapiaEstetica} onChange={(valor) => mudar("terapiaEstetica", valor)} valores={nomesEstetica} placeholder="Opcional" id="chamado-estetica" />
          <CampoBuscaLista label="Sala" value={form.sala} onChange={(valor) => mudar("sala", valor)} valores={salas.map((item) => item.nome)} placeholder="Selecione ou digite" id="chamado-sala" />
          <CampoBuscaLista label="TAA" value={form.taa} onChange={(valor) => mudar("taa", valor)} valores={taa.map((item) => item.nome)} placeholder="Selecione a situação" id="chamado-taa" />
        </div>
