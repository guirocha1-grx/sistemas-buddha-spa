...
export default function ConfirmacaoPagamentos() {
  ...
  const pixFiltrados = useMemo(() => {
    ...
    return (pixResultado?.pagamentos ?? []).filter((pagamento) =>
      [pagamento.pagador, pagamento.cpfCnpjPagador, pagamento.valor, pagamento.descricao, pagamento.endToEndId]
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <Card className="overflow-hidden border-primary/15 shadow-sm">
        ...
        <CardContent className="space-y-4 pt-5">
          ...
          {pixResultado && <><div className="relative"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input value={buscaPix} onChange={(event) => setBuscaPix(event.target.value)} className="pl-9" placeholder="Buscar por nome, CPF, valor ou identificador Pix" /></div><div className="max-h-[420px] overflow-auto rounded-lg border"><Table><TableHeader><TableRow><TableHead>Recebido em</TableHead><TableHead>Pagador</TableHead><TableHead>Valor</TableHead><TableHead>Identificação</TableHead></TableRow></TableHeader><TableBody>{pixFiltrados.length ? pixFiltrados.map((pagamento) => <TableRow key={pagamento.idTransacao}><TableCell className="whitespace-nowrap text-xs">{dataHora(pagamento.dataHora)}</TableCell><TableCell><p className="font-medium">{pagamento.pagador ?? "Não informado"}</p><p className="text-xs text-muted-foreground">CPF/CNPJ {mascaraDocumento(pagamento.cpfCnpjPagador)}</p></TableCell><TableCell className="whitespace-nowrap font-semibold text-emerald-700">{moeda(pagamento.valor)}</TableCell><TableCell className="max-w-36 truncate font-mono text-xs text-muted-foreground" title={pagamento.endToEndId ?? undefined}>{pagamento.endToEndId ?? "—"}</TableCell></TableRow>) : <TableRow><TableCell colSpan={4} className="h-24 text-center text-sm text-muted-foreground">Nenhum Pix encontrado para a busca informada.</TableCell></TableRow>}</TableBody></Table></div></>}
        </CardContent>
      </Card>
